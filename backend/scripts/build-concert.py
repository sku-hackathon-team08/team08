...
for g in layout["gates"]:
    ...
    gates.append(
        dict(
            id=uid(g["key"]),
            name=g["name"],
            position=dict(lng=lng, lat=lt),
            zoneId=uid(g["zoneKey"]) if g["zoneKey"] else None,
        )
    )
...
def material(name, color, emissive=False):
    ...
    m = dict(
        ...
        pbrMetallicRoughness=dict(
            baseColorFactor=c + [1], metallicFactor=0.15, roughnessFactor=0.75
        ),
        doubleSided=True,
    )
    if emissive:
        m["emissiveFactor"] = [v * 0.7 for v in c]
    idx = len(materials)
    materials.append(m)
    return idx


steel = material("Dark steel", "#263747")
silver = material("Truss", "#abb9c1")
deck = material("Stage deck", "#253142")
screen = material("LED mint", "#6befcb", True)
pink = material("LED coral", "#ed91b4", True)
white = material("Canopy", "#e7e6dd")
black = material("Speaker", "#131a20")
floor = material("Protective floor", "#465461")
aisle = material("Walkways", "#a3b0b4")
gold = material("Safety yellow", "#ebc573")
zone_mats = {z["key"]: material(z["name"], z["color"]) for z in layout["zones"]}


def box(name, x, y, z, w, d, h, mat, angle=0):
    nodes.append(
        dict(name=name, mesh=mat, center=[x, y, z], size=[w, d, h], angle=angle)
    )
...
for x in [-32, 32]:
    box("Perimeter circulation", x, 0, 0.03, 5, 69, 0.06, aisle)
for zone in layout["zones"]:
    x1, y1, x2, y2 = zone["boundsMeters"]
    box(
        "Section surface " + zone["key"],
        (x1 + x2) / 2,
        (y1 + y2) / 2,
        0.04,
        x2 - x1,
...
for zone in layout["zones"][:4]:
    ...
    while y < y2 - 1:
        for bank in [(x1 + 1, x1 + 10), (x2 - 10, x2 - 1)]:
            ...
            while x < bank[1]:
                box(
                    "Seat cushion", x, y, 0.49, 0.48, 0.46, 0.13, zone_mats[zone["key"]]
                )
                box(
                    "Seat back",
                    x,
                ...
                box("Seat support", x, y, 0.25, 0.09, 0.28, 0.5, steel)
                seat_count += 1
                x += 0.78
        y += 1.25
...
for x in [-18, 18]:
    ...
    box("Side LED", x, 43.4, 6, 5.5, 0.15, 8.5, pink)
    for y in [38.5, 49.5]:
        box("Roof column", x * 0.83, y, 7, 0.5, 0.5, 14, silver)
    for z in [3, 4, 5, 6, 7, 8]:
        box("Line array", x * 0.76, 37, z, 1.1, 1.2, 0.85, black)
for y in [38.5, 49.5]:
    for z in [13, 14]:
        box("Roof crossbeam", 0, y, z, 31, 0.35, 0.35, silver)
    for x in range(-15, 16, 3):
        box("Truss brace", x, y, 13.5, 0.2, 0.35, 1, silver)
for x in [-15, 15]:
    box("Roof sidebeam", x, 44, 13.5, 0.45, 12, 0.5, silver)
box("Stage canopy", 0, 44, 14.2, 32, 12, 0.18, black)
for x in range(-12, 13, 3):
    box("Light fixture", x, 38.2, 12.5, 0.6, 0.5, 0.45, pink if x % 2 else screen)
...
for x in [-4.5, 4.5]:
    for y in [-40, -34]:
        box("FOH post", x, y, 2.5, 0.18, 0.18, 5, silver)
box("FOH canopy", 0, -37, 5.1, 10, 7, 0.2, white)
for x in [-24, 24]:
    box("Operations cabin", x, -43, 1.3, 5, 5, 2.6, white)
    box("Operations roof", x, -43, 2.8, 5.5, 5.5, 0.25, screen)
# Low crowd barriers follow the exact zone rectangle edges.
for z in layout["zones"][:4]:
    x1, y1, x2, y2 = z["boundsMeters"]
    for y in [y1, y2]:
        box("Audience rail", (x1 + x2) / 2, y, 1.05, x2 - x1, 0.12, 0.12, silver)
        for i in range(int((x2 - x1) / 3) + 1):
            box("Rail post", x1 + i * 3, y, 0.55, 0.1, 0.1, 1.1, silver)
    # Leave a six-meter opening on the outer side of each zone.
    inner = x2 if x2 < 0 else x1
    box("Central aisle rail", inner, (y1 + y2) / 2, 1.05, 0.12, y2 - y1, 0.12, silver)
    outer = x1 if x2 < 0 else x2
    for y in [y1 + (y2 - y1 - 6) / 4, y2 - (y2 - y1 - 6) / 4]:
        box("Outer rail", outer, y, 1.05, 0.12, (y2 - y1 - 6) / 2, 0.12, silver)
for g in layout["gates"]:
    x, y = g["positionMeters"]
    for dy in [-2.5, 2.5]:
        box("Entry frame post", x, y + dy, 2, 0.3, 0.3, 4, silver)
    box("Entry header", x, y, 4, 0.4, 5.3, 0.45, screen)
...
for y in [38.5, 49.5]:
    for x in range(-15, 15, 3):
        beam("Roof diagonal", [x, y, 13], [x + 3, y, 14], 0.1, silver)
        beam("Roof diagonal", [x, y, 14], [x + 3, y, 13], 0.1, silver)
for x in [-15, 15]:
    for y in [38.5, 49.5]:
        for z in range(0, 12, 2):
            beam("Tower diagonal", [x - 0.22, y, z], [x + 0.22, y, z + 2], 0.08, silver)
# Delay speaker and follow-spot towers outside the audience blocks.
for x in [-33, 33]:
    for y in [-22, 22]:
        box("Tower ballast", x, y, 0.3, 2.2, 2.2, 0.6, steel)
        for dx in [-0.55, 0.55]:
            for dy in [-0.55, 0.55]:
                box("Delay tower leg", x + dx, y + dy, 5, 0.12, 0.12, 10, silver)
        ...
        box("Follow spot platform", x, y, 9, 2, 2, 0.18, steel)
        box("Follow spot", x, y, 9.5, 0.6, 1, 0.55, black)
        for z in [6, 7, 8]:
            box("Delay speaker", x, y + 0.8, z, 0.8, 0.6, 0.7, black)
# FOH mixing desks, monitor screens, rear service lane, check-in desks and queues.
for x in [-3, -1, 1, 3]:
    box("Mixing console", x, -35.5, 2.48, 1.4, 0.8, 0.15, steel)
    box("Console screen", x, -35.2, 2.85, 1, 0.12, 0.55, screen)
box("Rear service aisle", 0, -46, 0.05, 70, 3, 0.1, aisle)
for x in [-24, 24]:
    for dx in [-1, 1]:
        box("Operations desk", x + dx, -45.5, 0.85, 1.5, 0.7, 1.5, steel)
    for dy in [-1, 0, 1]:
        box("Queue rail", x, -49 + dy * 1.4, 1.1, 8, 0.12, 0.12, silver)
    box("Service wayfinding", x, -42, 4.2, 3, 0.2, 0.65, screen)
# Simple direction arrows made of strips on the circulation lanes.
for x in [-32, 0, 32]:
    for y in [-27, -12, 12, 25]:
        box("Aisle arrow stem", x, y, 0.09, 0.14, 1.4, 0.035, white)
        for sign in [-1, 1]:
            box(
                "Aisle arrow head",
                x + sign * 0.24,
...
def axes(part):
    if "start" in part:
        ...
        zz = unit(delta)
        xx = unit(cross([0, 1, 0] if abs(zz[2]) > 0.9 else [0, 0, 1], zz))
        yy = cross(zz, xx)
        return (
            [(a[i] + b[i]) / 2 for i in range(3)],
            [part["width"], part["width"], math.sqrt(sum(v * v for v in delta))],
            [xx, yy, zz],
        )
...
def accessor(values, fmt, typ, target, minimum=None, maximum=None):
    ...
    idx = len(views)
    views.append(
        dict(buffer=0, byteOffset=len(buffer), byteLength=len(raw), target=target)
    )
    buffer.extend(raw)
    entry = dict(
        bufferView=idx,
        componentType=5126 if fmt == "f" else 5125,
        count=len(values) // (3 if typ == "VEC3" else 1),
        type=typ,
    )
    if minimum is not None:
        entry.update(min=minimum, max=maximum)
...
for mat in range(len(materials)):
    ...
    for part in [v for v in nodes if v["mesh"] == mat]:
        ...
        for face_normal, vertices in faces:
            ...
            for v in vertices:
                q = [
                    center[i] + sum(basis[j][i] * v[j] * size[j] for j in range(3))
                    for i in range(3)
                ]
                pos.extend([q[0], q[2], -q[1]])
                normal.extend([nn[0], nn[2], -nn[1]])
    ...
    pa = accessor(
        ...
        "VEC3",
        34962,
        [min(pos[i::3]) for i in range(3)],
        [max(pos[i::3]) for i in range(3)],
    )
    na = accessor(normal, "f", "VEC3", 34962)
    ia = accessor(indices, "I", "SCALAR", 34963)
    render_nodes.append(dict(name=materials[mat]["name"], mesh=len(meshes)))
    meshes.append(
        dict(
            primitives=[
                dict(attributes=dict(POSITION=pa, NORMAL=na), indices=ia, material=mat)
            ]
        )
    )
model = dict(
    asset=dict(version="2.0", generator="team08 concert builder"),
    scene=0,
    scenes=[dict(nodes=list(range(len(render_nodes))))],
    nodes=render_nodes,
    meshes=meshes,
    materials=materials,
    ...
    accessors=accessors,
    extras=dict(
        componentCount=len(nodes),
        seatMarkers=seat_count,
        layoutVersion=layout["dataVersion"],
    ),
)
j = json.dumps(model, separators=(",", ":")).encode()
j += b" " * ((-len(j)) % 4)
payload = bytes(buffer)
payload += b"\0" * ((-len(payload)) % 4)
glb = (
    struct.pack("<4sII", b"glTF", 2, 12 + 8 + len(j) + 8 + len(payload))
    + struct.pack("<I4s", len(j), b"JSON")
    + j
    + struct.pack("<I4s", len(payload), b"BIN\0")
    + payload
)
OUT.mkdir(exist_ok=True, parents=True)
(OUT / "concert.glb").write_bytes(glb)
data = dict(
    eventId=uid("event"),
    name=layout["name"],
    status=layout["status"],
    dataVersion=layout["dataVersion"],
    center={k: anchor[k] for k in ["lng", "lat"]},
    zones=zones,
    gates=gates,
    demoPoint=dict(
        lng=layout["demoPoint"]["lng"],
        lat=layout["demoPoint"]["lat"],
        zoneId=uid(layout["demoPoint"]["zoneKey"]),
        surfaceOffsetMeters=layout["demoPoint"]["surfaceOffsetMeters"],
    ),
    model=dict(
        uri=f"/api/v1/demo/events/{uid('event')}/assets/concert.glb",
        position={k: anchor[k] for k in ["lng", "lat"]},
        headingDegrees=anchor["headingDegrees"],
        metersPerUnit=1,
        heightPolicy="FLAT_DEMO_PLANE_ABOVE_RENDERED_FOOTPRINT",
        nodeCount=len(render_nodes),
        componentCount=len(nodes),
        seatMarkers=seat_count,
    ),
)
(OUT / "seoul-worldcup.json").write_text(
...
features += [
    dict(
        type="Feature",
        id=g["id"],
        properties=dict(name=g["name"], kind="GATE", zoneId=g["zoneId"]),
        geometry=dict(
            type="Point", coordinates=[g["position"]["lng"], g["position"]["lat"]]
        ),
    )
    for g in gates
]
(OUT / "concert-zones.geojson").write_text(
    json.dumps(
        dict(
...
for z in zones:
    for i, xy in enumerate(z["geometry"]["coordinates"][0]):
        w.writerow(["ZONE", z["id"], z["name"], i, *xy])
for g in gates:
    w.writerow(
        ["GATE", g["id"], g["name"], 0, g["position"]["lng"], g["position"]["lat"]]
    )
(OUT / "concert-coordinates.csv").write_text("\ufeff" + s.getvalue())
print(
    f"Built GLB: {len(nodes)} parts, {len(glb)} bytes; {len(zones)} zones, {len(gates)} gates."
)
