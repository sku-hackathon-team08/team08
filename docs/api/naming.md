...
오류의 필드 위치에 포함되는 필드명도 외부 이름을 사용합니다. 중첩·배열 경로와 요청 위치는 [공통 오류 계약](errors.md#입력-검증-오류)의 location·path로 표현합니다.
...
축제 API는 [공통 표현 규칙](festival.md#표현-규칙)에 따라 미등록 키를 422로 거절합니다. 선언된 camelCase와 다른 snake_case 키도 해당하며 대체 입력으로 사용하지 않습니다. 현재 ApiModel의 HTTP 미등록 키 처리 기본 동작은 아직 이 정책으로 변경하지 않았습니다.
