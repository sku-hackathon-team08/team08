...
필터 `types`·`statuses`는 쉼표 구분 enum이며 빈 값/알 수 없는 값은 422입니다. 관리자 목록의 CANCELLED 필터도 거절합니다. 관리자 정렬은 `sort=recent|urgency`, 기본 recent입니다. 필터를 먼저 적용한 뒤 `sort=recent`는 미확인 우선·접수 시각 내림차순·ID 내림차순이고, `sort=urgency`는 위험도순이 최우선이며 같은 위험도 안에서만 미확인 우선·접수 시각 내림차순·ID 내림차순입니다(2026-09-12 확정 변경). 스태프는 최근순, 로그는 발생 시각·ID 오름차순입니다.
