...
| `type` | ReportType | 필수 |
| `urgency` | Urgency | 필수 |
| `position` | [Position](models.md#position) | 필수 |
...
[StaffReport](models.md#staffreport). 상태 RECEIVED, claimedBy/claimedAt/resolvedAt/cancelledAt=null. createdAt은 서버 최종 접수 시각입니다. Location은 `/api/v1/staff/reports/{id}`입니다.
...
[ReportDetail](models.md#reportdetail). 상태 RECEIVED, 담당 없음. type.source=ADMIN_SELECTED, urgency.source=AI_SUGGESTED입니다. `confirmedBy/confirmedAt`은 null이며 생성이 담당 배정·분류 확정을 대체하지 않습니다. positionSource=MAP_SELECTED입니다. 이후 claim으로 맡아야 합니다.
