...
> 현재 해커톤의 채택·구현 계약은 [해커톤 백엔드 API](hackathon.md)입니다. 이 문서는 이전 v0.1 검토안을 보존하며 아래 미정/제안이 현재 구현을 덮어쓰지 않습니다. 행사 신청·발급, GPS·구역 판정·접근 경로는 현재 범위에서 제외했습니다.
...
- 해커톤 확정: 공통 행사 코드·이름·진입 화면에서 선택한 role로 세션 발급. 별도 관리자 자격 증명을 요구하지 않음
- 재접속·세션 수명과 발급 재전송 정책은 별도 확인 후 구현
...
- 현재 구현은 422 요청 검증 변환을 제공하며 401·403·409 등의 업무 오류는 후속 구현
- 422를 제외한 이 문서의 업무 오류는 구현 예정 계약
...
- 해커톤에서는 별도 관리자 자격 증명을 받지 않습니다. 공통 행사 코드와 위 필드로 선택한 역할의 세션을 발급합니다.
...
> 현재 구현 범위에서는 GPS를 사용하지 않고 [고정 위치 계약](../features/report-intake.md#오늘-데모의-고정-위치)을 적용합니다. GPS 미수신 조사(TBD-03)는 범위에서 제외합니다. 입력/파일 제한·분석 실패 대안은 TBD-06입니다.
...
- 서버가 고정 위치와 현재 구역을 판정하며 고정 위치 출처 `DEMO_FIXED`를 기록하고 GPS 실측으로 표시하지 않음. 클라이언트가 임의로 출처를 선택하지 않음
...
export interface ReportCard {
  ...
  status: ReportStatus;
  position: Position;
  positionSource: "MAP_SELECTED" | "DEMO_FIXED"; // 스태프는 DEMO_FIXED, 관리자는 MAP_SELECTED
...
| A-08 | GPS 미사용·고정 위치 적용, 구역 밖 미정(TBD-06), 경로 미검증(TBD-09) | I03·O04 |
...
- 지원 정리·기간 산식 등 조건부 행위는 먼저 정책을 채택한 뒤 테스트 기대값을 확정함
