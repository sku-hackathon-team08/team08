...
| 콘텐츠 유형 | JSON: `application/json`, 음성: `multipart/form-data` | 검토안 |
| ID·시각 | UUID 문자열 / UTC RFC 3339 문자열 | 검토안 |
| 단위 | 초·미터를 필드명에 명시 | 검토안 |
| 응답 필드 | 별도 표시가 없으면 항상 존재 | 검토안 |
| 선택·빈 값 | `?`: 생략 가능, `null`: 명시적 빈 값, 빈 목록: `[]` | 검토안 |
| 성공 본문 | 모델 직접 반환; `{data: ...}` 래퍼 없음 | 검토안 |
| 204 | 본문 없음 | 적용 시 준수 |
| 미등록 입력 | 거절; 서버 소유자·역할·상태·구역·이력·출처 덮어쓰기 불가 | 검토안 |
| 필수 문자열 | 앞뒤 공백 제거 후 빈 값이면 422 | 검토안 |
| 길이·파일 상한 | 수치 미정 | A-03 |
...
- 모든 POST·PATCH에는 `Idempotency-Key` 문자열 헤더를 필수로 제안함
- 하나의 사용자 동작에서 생성한 키는 응답 유실 후에도 유지함
- 새로운 참여·재요청 같은 새 의도는 새 키를 사용함
- 키 형식은 UUID 문자열 제안

- 범위는 행사(없으면 신청 자격)·행위자·메서드·정규 경로·키
- 같은 키/같은 본문은 최초 상태 코드와 결과를 반환하고, 다른 본문은 409 IDEMPOTENCY_CONFLICT
- 처리중 재시도는 409 REQUEST_IN_PROGRESS로 구분함
- 파일 요청의 비교에는 파일 내용도 포함함
- 미완료 실패의 재시도·결과 보관 기간은 A-04에서 결정함
- 보관 기간을 정하지 않은 채 무기한 중복 방지를 보장하지 않음

- 처리 액션은 `expectedVersion:integer≥1`을 요구함
- 버전 불일치는 409 STALE_VERSION으로 거절함
- 업무 상태 변경과 필요한 이력은 함께 성공하거나 함께 실패해야 함
- 재전송 결과 복구는 새 버전 검사보다 먼저 수행하되 인증·접근 권한 검사는 생략하지 않음
...
- 인증·자원 범위 → 멱등 재응답 → 입력/버전·업무 조건 확인 순서를 적용하는 제안
...
- 모든 작업은 ADMIN 같은 행사, Idempotency-Key 및 expectedVersion이 필요
...
- 오류: 401/403/404/422 및 STALE_VERSION·INVALID_REPORT_STATE·공통 멱등성 오류. 다른 담당자의 완료/분류/해제는 403
...
- 다른 사람 참여 취소는 403, 이미 끝난 참여를 새 키로 취소하면 409
- 동일 Idempotency-Key 재시도는 원래 201/200을 복구함
...
export interface ApiError {
  ...
  detail: string;
  errors: unknown[]; // 필드 오류 구조 확정 후 좁힐 예정
}
```
