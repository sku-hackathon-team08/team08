...
- 모르는 행사: 공통 404 `NOT_FOUND`. 잘못된 UUID: FastAPI 기본 422.
...
- 허용 목록 외 파일명: FastAPI 기본 422. 임의 디렉터리를 정적으로 공개하지 않습니다.
...
프론트는 API 실패를 표시하며 번들에 내장된 행사 좌표로 되돌아가지 않습니다.
현재 업무 DB 테이블·행사 생성·코드 인증은 구현하지 않았습니다.
후속 제품 연동에서 인증된 행사 컨텍스트와 DB 저장을 연결하고 E05 계약을 별도로 확정합니다.
