...
- 역할·목적은 짧은 구절을 기본으로 적습니다. 이유·제약은 의미가 분명해지도록 필요한 만큼 문장으로 설명합니다. `~한다` 같은 종결형을 강제하지 않습니다.
...
이유를 풀어 쓰는 편이 이해하기 쉬우면 문장형 주석도 사용할 수 있습니다. 아래는 작성 방식의 예시입니다.

```python
# 중복 결제를 방지하기 위해 재시도 전에 처리 결과를 확인한다.
```
