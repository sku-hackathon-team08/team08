...
> 상태: 최소 앱·상태 확인 API·테스트·CI를 구성했습니다. DB 연결·세션과 시작/종료 처리, API 공통 스키마·입력 경계, 일반 HTTP 오류·422 공통 오류 변환을 구현했으며 [해커톤 API](../api/hackathon.md)·업무 모델·Alembic·OpenAI·PDF를 구현했습니다.
...
> 상태: 설정 로딩·URL 검증과 실제 DB 연결·SQLite 파일 생성·시작 실패 및 종료 정리를 구현했습니다. 업무 테이블·마이그레이션은 [DB 계약](../db/storage.md)을 따릅니다.
...
- 의존성은 commit하지 않습니다. 닫힐 때 미완료 트랜잭션은 정리되므로 저장 성공을 원하면 향후 합의할 업무 경계에서 명시적으로 완료해야 합니다. 현재 commit 책임은 [업무 트랜잭션](../db/storage.md#트랜잭션)을 따릅니다.

ORM 공통 기반은 models/base.py이며 업무 모델·마이그레이션은 [DB 계약](../db/storage.md)에 기록합니다. 범용 CRUD 프레임워크는 도입하지 않습니다.
...
> 상태: Python 표준 logging으로 DB 준비·정리 완료 로그와 환경별 레벨 설정을 구현했습니다. HTTP 서버 오류는 아래 기록 책임에 따라 처리하며 외부 로그 수집은 후속 작업입니다.
...
| ERROR | 앱 시작·종료 실패와 예상하지 못한 요청 오류는 Uvicorn이 예외·스택을 기록. 처리된 명시적 HTTP 500은 앱이 안전한 진단 기록 |
...
- 예상하지 못한 요청 오류는 안전한 500 응답을 만든 뒤 Starlette가 원래 예외를 다시 전파하므로 Uvicorn이 원인과 스택을 ERROR로 한 번 기록합니다. 핸들러에서 같은 예외를 중복 기록하지 않습니다. 서버 응답 스키마 검증 실패도 이 경로를 사용합니다.
- 명시적인 `HTTPException(500)`은 HTTP 핸들러에서 처리가 끝나므로 앱이 예외 원문을 제외한 진단을 ERROR로 한 번 기록합니다. 400·404·405는 새 서버 오류 로그를 남기지 않습니다. 클라이언트 응답은 [오류 계약](../api/errors.md)을 따릅니다.
- 예상하지 못한 예외의 Uvicorn 원본 스택에는 예외 메시지나 검증 입력이 포함될 수 있습니다. 응답에서 이를 가리는 것과 서버 로그 자동 마스킹은 별개이며 외부 로거의 예외 마스킹은 아직 구현하지 않았습니다.
...
- 축제 API의 미등록 입력 거절·타입 변환은 [확정 표현 규칙](../api/festival.md#표현-규칙)을 따릅니다. 현재 모델 기본 동작과 다른 부분은 #54에서 구현하며 Pydantic 기본 동작을 확정 정책으로 간주하지 않습니다.
- 필드 오류 경로는 [확정 오류 계약](../api/errors.md#입력-검증-오류)을 따르고 422 변환은 api/validation.py에서 구현했습니다.
...
> 상태: 공통 오류 계약의 구현 가이드. 일반 오류 스키마와 400·404·405·500 핸들러를 구현했습니다. 422 필드 오류 경로 계약은 #50에서 확정했으며 별도 ValidationErrorResponse 모델과 핸들러로 변환합니다.
...
| `schemas/errors.py` | `ApiModel`을 상속한 일반 오류 응답 모델 `ErrorResponse` |
| `api/errors.py` | HTTP 오류·예상하지 못한 예외를 HTTP 응답으로 변환. 업무 예외·필드 오류 변환은 후속 구현 |
| `main.py` | 예외 핸들러 등록과 OpenAPI의 공통 적용 범위·500 응답 조립 |
| 프론트 공통 API 클라이언트 | 실제 응답 검증·오류 정리, 네트워크 및 비JSON 응답의 기본 처리 |
| 프론트 화면 | 코드에 따른 동작, 필드 오류·일반 안내 표시 |

`main.py`에서 Starlette의 `HTTPException`과 일반 `Exception`에 핸들러를 등록합니다. Starlette 예외를 기준으로 등록하므로 FastAPI에서 직접 던진 오류와 라우팅이 생성한 오류를 함께 처리합니다. 별도 오류 미들웨어는 추가하지 않습니다.

`ErrorResponse`는 모든 필드를 필수로 선언하고 현재 일반 오류 범위만 모델링합니다. `errors: tuple[()]`는 빈 배열만 JSON으로 직렬화하고 OpenAPI에도 길이 0인 배열로 표현합니다. 필드 오류 항목을 임의로 정의하거나 422 구현에 이 모델을 재사용하지 않습니다.

핸들러는 [현재 적용 범위](../api/errors.md#적용-범위)에 해당하는 상태만 공통 응답으로 변환합니다. 나머지 HTTP 상태는 FastAPI의 `http_exception_handler()`에 위임해 원래 상태와 헤더를 유지합니다. `RequestValidationError` 핸들러는 재정의하지 않고, 서버 응답 검증 실패는 일반 서버 오류 경로로 처리합니다. `Allow` 등 오류 의미의 헤더는 보존하며 `Content-Type`·`Content-Length`·`Content-Encoding`은 새 JSON 표현에 맞게 설정하거나 제거합니다. HEAD에는 본문을 보내지 않습니다.

HTTP 400·404·405를 명시적으로 발생시킬 때 `detail`에는 공개 가능한 문자열만 넣습니다. `str(exc)`, SQL, 요청 원문을 전달하지 않습니다. HTTP 500과 예상하지 못한 예외의 설명은 항상 일반 안내로 대체하며 로그 기록 책임은 [오류 기록과 노출 범위](#오류-기록과-노출-범위)를 따릅니다.

OpenAPI의 전역 응답 선언은 일반 서버 오류용 500만 연결합니다. 기능에서 실제로 발생하는 400·404 등은 해당 라우트의 `responses`에 선언하고, 발생 조건은 기능별 API 명세에 기록합니다. 미등록 경로·잘못된 메서드의 오류와 422 적용 범위는 OpenAPI의 앱 설명에서 안내합니다. 스키마와 실제 응답은 [오류 테스트](testing.md#백엔드-테스트)로 대조합니다.
