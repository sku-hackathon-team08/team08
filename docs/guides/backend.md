...
> 상태: 최소 앱·상태 확인 API·테스트·CI를 구성했습니다. DB 연결·세션과 시작/종료 처리, API 공통 스키마·입력 경계, 일반 HTTP 오류 처리를 구현했으며 422 공통 오류 변환·서비스 API·업무 모델은 후속 작업입니다.
...
- 필드 오류 경로는 [확정 오류 계약](../api/errors.md#입력-검증-오류)을 따르고 422 변환은 #54에서 구현합니다.
...
> 상태: 공통 오류 계약의 구현 가이드. 일반 오류 스키마와 400·404·405·500 핸들러를 구현했습니다. 422는 FastAPI 기본 응답을 유지하며 필드 오류 경로 계약은 #50에서 확정했으며 변환은 #54에서 구현합니다.
...
OpenAPI의 전역 응답 선언은 일반 서버 오류용 500만 연결합니다. 기능에서 실제로 발생하는 400·404 등은 해당 라우트의 `responses`에 선언하고, 발생 조건은 기능별 API 명세에 기록합니다. 미등록 경로·잘못된 메서드의 오류와 422 미적용 상태는 OpenAPI의 앱 설명에서 안내합니다. 스키마와 실제 응답은 [오류 테스트](testing.md#백엔드-테스트)로 대조합니다.
