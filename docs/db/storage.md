...
완료 로그의 처리시간은 reports.claimed_at부터 log.occurred_at까지입니다. 현재 전이는 완료 뒤 재개방/재배정을 허용하지 않으므로 최종 배정 시각이 보존됩니다. 향후 재개방을 도입하면 이 산식을 과거 담당 구간 저장 방식과 함께 변경해야 합니다. 이번 작업은 기존 로그와 컬럼을 사용하며 스키마 변경·데이터 소급 생성은 없습니다. 로그가 없는 과거 행위를 현재 담당자로 추정해 채우지 않습니다.
