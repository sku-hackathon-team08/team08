...
## 관리자 처리 리포트의 기존 기록 사용

`report_logs`의 actor_id·action·occurred_at·changes·note로 본인 처리 이력을 조회합니다. 담당 해제 후 reports.claimed_by/claimed_at이 초기화되어도 과거 담당 로그는 남습니다. 완료·취소도 행위자 ID와 메모를 같은 트랜잭션에 저장합니다. 재배정 후 과거 담당자에게 최종 완료 실적을 귀속하지 않습니다.

완료 로그의 처리시간은 reports.claimed_at부터 reports.resolved_at까지입니다. 현재 전이는 완료 뒤 재개방/재배정을 허용하지 않으므로 최종 배정 시각이 보존됩니다. 향후 재개방을 도입하면 이 산식을 과거 담당 구간 저장 방식과 함께 변경해야 합니다. 이번 작업은 기존 로그와 컬럼을 사용하며 스키마 변경·데이터 소급 생성은 없습니다. 로그가 없는 과거 행위를 현재 담당자로 추정해 채우지 않습니다.
